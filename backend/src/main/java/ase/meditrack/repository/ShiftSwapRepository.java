...
public interface ShiftSwapRepository extends JpaRepository<ShiftSwap, UUID> {
     ...
     * This includes own created requests, as well as the suggestions from other users.
     *
     * @param shiftId from the shift
     */
    @Modifying
    @Transactional
    @Query("DELETE FROM shift_swap s WHERE s.requestedShift.id = :shiftId "
            + "OR s.suggestedShift.id = :shiftId")
